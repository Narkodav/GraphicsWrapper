...
namespace Graphics {
    ...
    concept DescriptorDefinition = requires {
        ...
        { T::getDescriptorBindingFlags() } -> std::same_as<vk::DescriptorBindingFlags>;
    };
    ...
    public:
        ...
        auto enumerateBindingFlags() const
        {
            std::vector<vk::DescriptorBindingFlags> bindingFlags;
            bindingFlags.reserve(this->size());
            this->iterateComplete([&]<DescriptorDefinition T>(const T & elem) {
                bindingFlags.push_back(elem.getDescriptorBindingFlags());
            });
            return bindingFlags;
        }
    };
    ...
    struct BindlessImageSamplerDefinition {
        static constexpr size_t BINDING_COUNT = 1;
