...
namespace Graphics {
    ...
    MappedMemory::MappedMemory(const Instance& instance, const Device& device,
    ...
    bool MappedMemory::bindBuffer(const Instance& instance, const Device& device, const Buffer& buffer, size_t offset) {
        if (!m_initialized)
            return false;
    ...
    bool MappedMemory::bindImage(const Instance& instance, const Device& device,
