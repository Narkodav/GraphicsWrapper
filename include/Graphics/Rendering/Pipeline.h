...
namespace Graphics {
    ...
    private:
        ...
        std::pair<std::vector<vk::PipelineShaderStageCreateInfo>,
            ...
            if (m_shaders.hasShader<Shader::Type::TessellationControl>()) {
                if (!m_shaders.hasShader<Shader::Type::TessellationEvaluation>()) {
                    throw std::runtime_error("Both tessellation shaders must be provided");
                }
                result.first.push_back(createShaderStage<Shader::Type::TessellationControl>(
                    vk::ShaderStageFlagBits::eTessellationControl));
                result.first.push_back(createShaderStage<Shader::Type::TessellationEvaluation>(
    ...
    public:
        ...
        Pipeline(const Instance& instance, const Device& device, const RenderPass& renderPass,
        ...
        Pipeline& operator=(Pipeline&& other) noexcept
        {
            ...
            assert(!m_initialized && "Pipeline::operator=() - Pipeline already initialized");
        ...
        ~Pipeline() { assert(!m_initialized && "Pipeline was not destroyed!"); };
