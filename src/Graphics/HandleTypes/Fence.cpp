...
namespace Graphics
{
    void Fence::create(const DeviceFunctionTable& functions, DeviceRef device,
    ...
    void Fence::destroy(const DeviceFunctionTable& functions, DeviceRef device)
    {
    ...
    void FenceRef::wait(const DeviceFunctionTable& functions, DeviceRef device,
        size_t timeout /*= std::numeric_limits<size_t>::max()*/) const
    {
        FenceRef::wait(functions, device, this, 1, timeout, true);
    }

    void FenceRef::reset(const DeviceFunctionTable& functions, DeviceRef device)
    {
